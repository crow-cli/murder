...
if [[ -z "$__murder_installed" ]]; then
    __murder_installed=1
    trap '__murder_preexec "$BASH_COMMAND"' DEBUG
    PROMPT_COMMAND="__murder_prompt_command${PROMPT_COMMAND:+;$PROMPT_COMMAND}"
fi
"#;
...
if [[ -z "$__murder_installed" ]]; then
    ...
    add-zsh-hook preexec __murder_preexec
    add-zsh-hook precmd __murder_precmd
fi
"#;

const FISH_INTEGRATION: &str = r#"
function __murder_preexec --on-event fish_preexec
    printf '\e]633;C\a'
    printf '\e]633;E;%s\a' "$argv[1]"
end
function __murder_postexec --on-event fish_postexec
...
if (-not $__murder_installed) {
    $__murder_installed = $true
    $__murder_original_prompt = $function:prompt
    function prompt {
        $exitCode = $LASTEXITCODE
        [Console]::Write("`e]633;D;$exitCode`a")
        [Console]::Write("`e]633;A`a")
        [Console]::Write("`e]633;P;Cwd=$PWD`a")
        & $__murder_original_prompt
    }
